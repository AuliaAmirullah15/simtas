						...
						<div class=\"alert alert-success\" role=\"alert\">
							<strong>Well done! </strong>". $message."</div>"; } }?>
								<div class="section-header">
										<ol class="breadcrumb">
											<li><a href="<?php echo base_url('tugas/mahasiswa_seminar');?>">Jadwal Seminar</a></li>
									...
									<table id="datatable1" class="table table-striped table-hover">
										<thead>
											<tr>
                                                ...
                                                <?php if ($_SESSION['level'] == '4' or $_SESSION['level'] == '5') { ?>
                                                    ...
                                                    <th>Kelayakan Seminar/Sidang</th>
										...
										<tbody>
											...
											<tr>
                                                ...
                                                <?php echo anchor('tugas/delete_mahasiswa_sidang/'.$b->nim.'/'.$b->id_jadwal_seminar,'<button type="button" class="btn btn-warning">Hapus</button>'); ?>
                                                <?php } ?>
												...
												<?php echo anchor('tugas/form_penilaian/'.$b->nim.'/'.$b->id_jadwal_seminar.'/'. $b->id_pengajuan,'<button type="button" class="btn btn-info">Form Penilaian</button>'); if($seminar == 'sidang') { ?>
												...
												<?php } if($seminar == 'seminar proposal' AND $_SESSION['level'] == '2') { ?>
                                                    <a href="<?php echo base_url('Tugas/penilaian_sempro/'.$b->nim.'/'. $b->id_pengajuan.'/'. $id); ?>"><button class="btn btn-danger">Nilai & Catatan</button></a>
                                                <?php } ?>
                                                    
                                                    <?php if($seminar == 'seminar proposal' AND ($_SESSION['level'] == '4' or $_SESSION['level'] == '5')) { echo "
                                                    <td>". ucwords($b->status_kelayakan) ."
                                                    
                                                    <td>". anchor('tugas/edit_sempro/layak/'.$b->nim.'/'. $id_seminarnya.'/'.$b->id_pengajuan,'<button type="button" class="btn btn-success"  OnClick="return confirm(\'Yakin Data ini Layak ?\')">Layak</button>')."
																	". anchor('tugas/edit_sempro/tdk_layak/'.$b->nim.'/'. $id_seminarnya.'/'.$b->id_pengajuan,'<button type="button" class="btn btn-warning" OnClick="return confirm(\'Yakin Data ini Tidak Layak ?\')">Tidak Layak</button>')."</td>";
                                                    
                                                    } ?>
												<?php if($seminar != 'seminar proposal' AND $seminar != 'sidang' AND ($b->pembimbing1 == $kd_dsn or $b->pembimbing2 == $kd_dsn or $b->pembanding1 == $kd_dsn or $b->pembanding2 == $kd_dsn) AND ($kd_dsn != '' or $kd_dsn != null) ) { ?>
<!--												<?php echo anchor('tugas/nilai/'.$b->nim.'/'.$seminar.'/'.$b->id_jadwal_seminar,'<button type="button" class="btn btn-danger">Nilai</button>'); ?>-->
                                                    <?php echo anchor('tugas/penilaian_semhas/'.$b->nim.'/'.$b->id_jadwal_seminar.'/'. $b->id_pengajuan,'<button type="button" class="btn btn-danger"> Nilai </button>'); ?>
                                                    
                                                     <?php echo anchor('tugas/catatan_semhas/'.$b->nim.'/'.$b->id_jadwal_seminar.'/'. $b->id_pengajuan,'<button type="button" class="btn btn-warning"> Catatan Semhas </button>'); ?>
                                                    
                                                    <?php } if($seminar != 'seminar proposal' AND $seminar != 'seminar hasil' AND ($b->pembimbing1 == $kd_dsn or $b->pembimbing2 == $kd_dsn or $b->pembanding1 == $kd_dsn or $b->pembanding2 == $kd_dsn) AND ($kd_dsn != '' or $kd_dsn != null) ) { ?>
                                                    <?php echo anchor('tugas/penilaian_sidang/'.$b->nim.'/'.$b->id_jadwal_seminar.'/'. $b->id_pengajuan,'<button type="button" class="btn btn-warning"> Nilai </button>'); ?>

												<?php } if($seminar == 'seminar hasil' AND ($_SESSION['level'] == '4' or $_SESSION['level'] == '5')) { ?>
                                                    
                                                    <td><?php if($b->status_kelayakan == 'layak') { echo "Setuju";} else if($b->status_kelayakan == 'tidak layak') { echo "Ulang"; } else { echo ucwords($b->status_kelayakan) ;} ?></td>
                                                    <td><a href="<?php echo base_url('Tugas/proses_status_semhas/layak/'. $b->nim.'/'. $b->id_pengajuan.'/'. $id_seminarnya.'/'. $seminar);?>"><button class="btn btn-success">Setuju</button></a>
                                                    <a href="<?php echo base_url('Tugas/proses_status_semhas/tdk_layak/'. $b->nim.'/'. $b->id_pengajuan.'/'. $id_seminarnya.'/'. $seminar);?>"><button class="btn btn-warning">Ulang</button></a>
                                                    </td>
                                                    
                                                <?php } if($seminar == 'sidang' AND ($_SESSION['level'] == '4' or $_SESSION['level'] == '5')) { ?>
                                                    
                                                    <td><?php if($b->status_kelayakan == 'layak') { echo "Lulus";} else if($b->status_kelayakan == 'tidak layak') { echo "Ulang"; } else { echo ucwords($b->status_kelayakan) ;} ?></td>
                                                    <td><a href="<?php echo base_url('Tugas/proses_status_sidang/layak/'. $b->nim.'/'. $b->id_pengajuan.'/'. $id_seminarnya.'/'. $seminar);?>"><button class="btn btn-success">Lulus</button></a>
                                                    <a href="<?php echo base_url('Tugas/proses_status_sidang/tdk_layak/'. $b->nim.'/'. $b->id_pengajuan.'/'. $id_seminarnya.'/'. $seminar);?>"><button class="btn btn-warning">Ulang</button></a>
                                                    </td>
                                                    
                                                <?php } if($seminar == 'seminar hasil' AND $_SESSION['level'] == '1') { ?>
