...
<?php
	$nim = $data[0]; 
	$seminar = preg_replace('/%20/',' ', $data[1]);
	$id = $data[2];

	foreach($nilai as $n)
	{
		$pembimbing1 = $n->pembimbing1;
		$pembimbing2 = $n->pembimbing2;
		$pembanding1 = $n->pembanding1;
		$pembanding2 = $n->pembanding2;
	}
	...
	foreach($total as $t)
	{
		if($seminar == 'sidang')
		{
			$total = $t->nilai_sidang;
		}

		else
		{
			$total = $t->nilai_semhas;
		}
	}
	
	foreach($nTotal as $nt)
	{
		$total_all = $nt->total;
		$grade = $nt->grade;
	}
...
?>
		...
		<div id="base">
			...
			<div class="section-header">
						<ol class="breadcrumb">
							<li class="active">Input Nilai <?= $seminar ?> </li>
						</ol>
					</div>
			...
			<div class="card">
							...
							<?php $i =1; ?>
								<form class="form-horizontal" role="form" method="post" action="<?php echo base_url().'tugas/post_nilai/'.$nim.'/'.$seminar.'/'.$id;?>" >
										...
										<div class="col-sm-10">
											<div class="input-group">
												<div class="input-group-content">
													...
													<input type="text" class="form-control" name="<?php if($i==1) {echo 'pembimbing1';} else{echo 'pembimbing2';}?>" value="<?php if($i==1){echo $pembimbing1;} else{echo $pembimbing2;}?>" placeholder="100.000">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<div class="input-group">
												<div class="input-group-content">
													<input type="text" class="form-control" name="<?php if($i==1) {echo 'pembanding1';} else{echo 'pembanding2';}?>" value="<?php if($i==1){echo $pembanding1;} else{echo $pembanding2;}?>" placeholder="100.000">
												</div>
											</div>
										</div>
									...
									<div class="form-group">
										<label for="total" class="col-sm-2 control-label">Total Nilai <?= $seminar ?></label>
										<div class="col-sm-10">
											<div class="input-group">
												<div class="input-group-content">
													<input type="text" class="form-control" name="nilai" value="<?= $total ?>" placeholder="100.000">
												</div>
											</div>
										</div>
									</div>
									...
									<div class="form-group">
										<label for="total" class="col-sm-2 control-label">Total Nilai Tugas Akhir</label>
										<div class="col-sm-10">
											<div class="input-group">
												<div class="input-group-content">
													<input type="text" class="form-control" name="total" value="<?= $total_all ?>" placeholder="100.000">
												</div>
											</div>
										</div>
									</div>
									...
									<div class="form-group">
										<label for="total" class="col-sm-2 control-label">Grade</label>
										<div class="col-sm-10">
											<div class="input-group">
												<div class="input-group-content">
													<input type="text" class="form-control" name="grade" value="<?= $grade ?>" placeholder="B+">
												</div>
											</div>
										</div>
									</div>
