...
@media print {
	...
	<body class="menubar-hoverable header-fixed ">
		...
		<?php
           ...
           foreach($penilaian_sempro as $ps) { 
            
                $id = $d->id;
                $judul_skripsi = $ps->judul_skripsi;
                $catatan_judul_skripsi = $ps->catatan_judul_skripsi;
                $latar_belakang = $ps->latar_belakang;
                $catatan_latar_belakang = $ps->catatan_latar_belakang;
                $rumusan_masalah = $ps->rumusan_masalah;
                $catatan_rumusan_masalah = $ps->catatan_rumusan_masalah;
                $landasan_teori = $ps->landasan_teori;
                $catatan_landasan_teori = $ps->catatan_landasan_teori;
                $penelitian_terdahulu = $ps->penelitian_terdahulu;
                $catatan_penelitian_terdahulu = $ps->catatan_penelitian_terdahulu;
                $data_digunakan = $ps->data_digunakan;
                $catatan_data_digunakan = $ps->catatan_data_digunakan;
                $metodologi = $ps->metodologi;
                $catatan_metodologi = $ps->catatan_metodologi;
                $arsitektur_umum = $ps->arsitektur_umum;
                $catatan_arsitektur_umum = $ps->catatan_arsitektur_umum;
                $kelayakan_sempro = $ps->kelayakan_sempro;
                $dsn = $ps->Nama_dosen;
 }
            }
            
            if($_SESSION['level'] == '2' and $exceptional_dosen == null){
            foreach($dosen_penilai as $dp)
            {
                $dsn = $dp->Nama_dosen; 
            }
            }
			...
			<div id="content">
				<section>
					<div class="section-header">
						<ol class="breadcrumb">
							<li class="active">Form Penilaian Kelayakan Isi Proposal</li>
						</ol>
					</div>
							...
							<div class="col-lg-12">
								<div class="card card-printable style-default-light">
									...
									<div class="card-body style-default-bright">
                                        ...
                                        <?php if($exceptional_dosen == null and $penilaian_sempro == null) { ?>
										<div class="row">
											<div class="col-xs-2">
